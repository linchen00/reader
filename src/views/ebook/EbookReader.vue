...
<script>
  ...
  import {saveFontSize, saveFontFamily} from "utils/localStorage";
    ...
    methods: {
      ...
      initFontFamily() {
        ...
        if (!fontFamily) {
          ...
          this.setDefaultFontFamiy(fontFamily)
        }
      },
      initFontSize() {
      ...
      initGesture() {
        ...
        this.rendition.on('touchend', (event) => {
          const offStartX = event.changedTouches[0].clientX - this.touchStartX
          const time = event.timeStamp - this.touchStartTime
          if (time < 500 && offStartX > 40) {
            this.nextPage()
          } else if (time < 500 && offStartX < -40) {
            this.prevPage()
          } else {
            this.toggleTitleAdnMenu()
      ...
      initEpub() {
        const url = 'http://192.168.0.107:8888/epub/' + this.fileName + '.epub'
